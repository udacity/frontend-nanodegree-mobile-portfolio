...
function getAdj(x){
  return adjToArr[x] || adjToArr["scientific"];
}
...
function getNoun(y) {
  return nonToArr[y] || nonToArr["scifi"];
}
...
var pizzaSizeLabel = document.querySelector("#pizzaSize");
...
var resizePizzas = function(size) {
  ...
  var newWidth = sizeToWidth[size] * windowWidth + 'px';

  // Changes the value for the size of the pizza above the slider
  pizzaSizeLabel.innerHTML = sizeToLabel[size];
...
function updatePositions() {
  ...
  for (i = 0; i < 5; i++) {
    phases[i] = Math.sin(scrollTop + i) * 100;
  }
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  var movingPizzasNumber = 30;
  var movingPizzasDiv = document.querySelector("#movingPizzas1");

  for (var i = 0; i < movingPizzasNumber; i++) {
