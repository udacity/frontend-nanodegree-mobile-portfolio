...
/**
 * Create a function to return a random ingredient from an ingredient category list.
 * This consolidated the below functions into a call to "selectRandom".
 * @param   {String}   itemType Name of ingredient category list to use.
 * @returns {Function} A function that returns a random item from selected list.
 */
...
/**
 * Creates an html list item.
 * @param   {String} string A string to add to an html list.
 * @returns {String} HTML code for a list item containing the string.
 */
 ...
 * @returns {String} HTML code for a list of items.
 */
...
/**
 * Creates a DOM element for each pizza on the menu.
 * @param   {Number} i Index number for the pizza DOM element.
 * @returns {Object} DOM element for a pizza menu item.
 */
var pizzaElementGenerator = function(i) {
    ...
    pizzaImage.srcset = "images/pizza.webp";
...
/**
 * Responds to slider change by changing slider label and resizing pizza menu divs.
 * @param   {String}   size Value returned from slider.
 */
var resizePizzas = function(size) { 
     ...
     * @param   {Object} elem DOM element (div)
     * @param   {String} size Value returned from slider.
     * @returns {Number} Size multiplier to apply to div.
     */
    ...
    /**
     * Iterates over all the pizza menu divs and changes their width.
     * Using local pizzas array instead of a document query.
     * `dx` and `newwidth` calculations removed from loop.
     * @param {String} size Value returned from slider.
     */
...
/**
 * Array of pizza menu div elements.
 * @type {Array}
 */
...
/**
 * Translates the X position of sliding background pizza images.
 * Repeated document queries and timing API logging were removed.
 * CSS position update changed to transform:translate update.
 */
...
/**
 * Stores the last used scrollTop value.
 * @type {Number}
 * @default 0
 */
var offset = 0;
...
/**
 * Used to restrict rAF to run only when necessary.
 * @type {Boolean}
 * @default false
 */
...
/**
 * Called when scrolling is detected and queues one rAF at a time.
 */
function onScroll() { 
    if (!updateRunning) {
...
/**
 * Array of pizza image elements.
 * @type {Array}
 */
var items = [];

/**
 * Create the background sliding pizzas when the page loads.
 * Removed `basicLeft` style attribute during switch to using transform.
 */
function loadBackgroundPizzas() {
...
document.addEventListener('DOMContentLoaded', loadBackgroundPizzas);